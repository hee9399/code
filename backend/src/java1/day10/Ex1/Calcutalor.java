...
public class Calcutalor {
	...
	int plusInt(int x , int y) { // 3. 리턴값/매개변수 이 있는 메소드 리턴o 매개변수 o=2개
		// powerOn(); // 호출할 메소드가 객체내부에 존재하므로 호출 가능
		// void를 제외한 리턴타입 있을경우 return 키워드 필수
		
		int resuslt = x + y; // 매개변수로 부터 전달받은 2개의 변수를 더하기
		return resuslt; // 호출 했던 곳으로 더하기 한 결과를 전달한다!!
		
	}
	
	double divide(int x , int y) { // 4. 리턴값/매개변수 이 있는 메소드 
