...
public class Ex1_참조타입 {
	...
	// 배열{1 , 2 , 3}을 생성[힙영역에 메모리할당] 하고 [활당된주소를] 스택영역변수에 대입
	...
	String kind1 = "자동차"; // 스택영역 : kind1			힙영역 : "자동차" 객체 [32번지] 
...
/*
	자바 데이터 타입 : 변수 선언할때 사용되는 타입 
		...
		- 참조타입 : 배열 , 열거 , 클래스[String , Scanner , Random , System 등] , 인터페이스 
			- 변수에 힙메모리 생성된 메로리를 주소/번지 참조 
			
	JAM 메모리	
		
		스택영역 									힙영역
		-스레드(main함수)마다 별도구역					-객체가 생성되는 영역
		-함수(프레임)마다 별도구역							- 스택/메소드 영역에 참조 할수 있
			-지역변수									- 객체,배열,인터페이스 메모리 생성
			-함수 정의x 호출시 프레임 생성					- new 연산자 : 타입/클래스 기반으로 메모리생성후 주소/번지 반환
			-함수(프레임) 종료되면 제거
	----------------------------------------------------------------------------- 
	
	int age = 리터럴(이미정해져있는 해당소스코드의 고정값);
	String name	= 100번지		"신용권" : String 클래스 기반으로 만들어진 객체(속성,메소드/행동)
							100번지
	-----------------------------------------------------------------------------
	스택변수에 저장된 기본타입값 또는 참조 주소 비교
	==
	!= 
	같다 같지않다는 객체안에있는상태는 비교불가능하고 객체의 주소/스택 만 비교할수있다
*/
