...
public class Ex2_문자열 {
	public static void main(String[] args) {
		...
		String name1 = "홍길동"; 
		String name2 = "홍길동";
			// ????? : 두 스택영역의 지역변수가 참조하는 주소는 같을까? 를 생각해볼께요
		...
		char sex = ssn.charAt(6);	  // 9[0]5[1]0[2]6[3]2[4]4[5]1[6]2[7]3[8]0[9]1[10]2[11]3[12]
...
/*
	...
	자바 문자열에서 제공해주는 라이브러리=미리 만들어진 함수들
		...
		3. 문자열객체.length()				: 문자열에서 문자의 개수 반환 
			vs 배열명.length
		4. 문자열객체.replace("기본문자열" , "새로운문자열")	: 	문자열내 기존문자열이 존재하면 새로운문자열로 치환해서 반환
		5. 
			문자열객체.substring( 인덱스 )		: 해당 인덱스 부터 마지막인덱스까지 문자열 추출 
			문자열객체.substring( 시작인덱스 , 마지막인덱스 ) : 시작 인덱스 부터 마지막인덱스까지 문자열 추출
