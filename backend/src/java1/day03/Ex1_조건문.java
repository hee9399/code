...
public class Ex1_조건문 { // class start
	...
	public static void main(String[] args) { // main start
		...
		// [ p.111]
				...
				switch(num) { // num 변수를 연결해서 제어 , 위에처럼 비교 불가능 , 쓰는이유는 경우의수의 값이 정해져있지않은경우 숫자인식이 빠르니까 사용한다
				...
				if(score3 >= 90) { // 점수가 90점 이상이면
					...
					else {grade = "B";} // [94~90] 이면서 95점 미만이면 A 대입
					
				}else { // 점수가 90점 미만이면 
					
					if(score3 >= 95 ) {grade = "B+";} // [89~85] 이면서 85점 이상이면 B+ 
...
/*
	if : 조건에 따른 흐름 제어 
		...
		* if = 조건에 따른 true/false 실행 
			- s
 		* switch = 변수 값에 따른 실행
 			- 경우의 수 의 값이 정해져 있는 경우 / 범위 x
 			- break; : switch {} 만약에 해당 case 실행후  멈춤
*/
