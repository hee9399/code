package java2.day27_컬렉션프레임워크2.Ex3_HashMap;
...
public static void main(String[] args) {
		...
		Thread threadB = new Thread() { // 1. 스레드2 선언
			public void run() {
		...
		// main스레드는 다른 두 스레드가 작업(저장)이 끝날때까지 main스레드는 잠시 대기
		try {threadA.join(); threadB.join();}catch (Exception e) {	}
		
		System.out.println("map 객체수 : "+ map.size());
		System.out.println("map 상태 : " +map); // 두스레드가 저장을 하기전에 main스레드가 출력 먼저 했기 때문에
