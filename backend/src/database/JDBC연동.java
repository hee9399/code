	...
	public static void main(String[] args) {
	 ...
	 try { // * JDBC 드라이버 클래스들의 예외처리 ]
		...
		Class.forName("com.mysql.cj.jdbc.Driver"); // mysql 기준
			// 만약에 해당 JDBC 드라이버 클래스가 검색이 실패하면 예외가 발생한다. 그래서 예외처리를 해야한다
		// 3. DB 연결 [ DriverManager.getConnection() 메소드가 연결 성공하면 DB와연결된 객체 리턴 --> 만약에 실패하면 예외발생 -> 예외 처리
		...
		/*
			...
			// 1. DB주소 : 1. jdbc : oracle : thin : @localhost : 1521/orcl
...
/*
  		예외 처리 :
  			...
  			- try{} catch{ }
  				...
  				catch( 에외클래스명 객체변수명 ) { 예외가 발생했을때 실행할코드 }
