...
public class EmailService { // 이메일 전송에 관련된 클래스. 
	...
	// dto처럼 이동객체가 아니고 서비스를위한 클래스파일이다.
	...
	public boolean authsend( String toEmail , String contentHTML) {
		...
		// 1. 호스팅 설정[ 네이버 기준 ]
			// 키와 값으로 구성된 map 컬렉션 중 하나이다. , 어떠한 설정데이터를 객체화 시킬때 많이 사용한다.
		...
		// 2. 인증처리 [ 네이버기준에 보내는 사람의 정보가 맞는지 틀린지 ]
			
			// 인증할 객체
		// Authenticator - 라이브러리가 지원해주는 인증 객체 
		Authenticator authenticator  = new Authenticator() {
		
			// 패스워드 검증 함수 [ 구현 ]
				// .getPasswordAuthentication() : 패스워드 검증[인증] 메소드 구현 , 오버라이드해야함 
...
- 자바에서 이메일 전송 보내기 
	1. 이메일을 전송할 도메인 필요 [ 네이버 , 다음 , 구글 등등 ]
		[ 예시는 네이버로 하겠음 ( git : 패스워드 감추거나 private 저장소 ) , 2차 비밀번호는 불가능 ]
		- 네이버 로그인
		1. 메일 메뉴 
		2. 메뉴 -> 환경설정 -> POP3/IMTP 설정 
		
			[ POP3/IMTP ] 
				- IMAP/SMTP 사용 : 사용함
				[ 스마트폰 메일 애플리케이션 계정 설정 ] 확인 
					SMTP 서버명 : SMTP 사용 : 사용함 
						[ 스마트폰 메일 애플리케이션 계정 설정 ] 확인 
							c 서버명 : smtp.naver.com
	...
	3. 메인 전송 함수 만들기.
		...
		2. 보내는 사람의 계정 인증객체 
			...
			@Override
			protected PasswordAuthentication getPasswordAuthentication() {
				// return new PasswordAuthentication("보내는사람이메일", "보내는사람의이메일패스워드");					
				return new PasswordAuthentication("보내는사람의이메일", "보내는사람의비밀번호");
			}
