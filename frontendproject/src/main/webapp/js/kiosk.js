...
// ------------------------------------------------------------- //
	...
	// 1. 주문(주문내역) 배열
		/* 
			객체를 어떻게 담을껀지
			주문 {주문번호 : , 주문날짜 : , 결제금액 : , 주문제품 : [ ] , 상태 :0[주문요청] 1[주문완료] 2[주문츼소]}
		*/
...
// 6. 카트내 버거들을 출력하는  [실행조건 : 1. 카트내 제품이 등록되면(5번 함수 하단 호출) 2. 카트내 제품 취소되면]
function cartPrint(){
	// 1.어디에 // 인수판단 :: 모든 카트내 제품출력
	...
	let html = ``; 
		...
		for(let i = 0 ; i<cartList.length ; i++){
			// 1. 카트내 버거의 인덱스 
			let burgerIndex = cartList[i]; console.log(cartList);
...
function productOrder(){
	...
	let ono = [orderList.length-1].ono; // 주문배열내 마지막 주문의 번호
	// * 카트(전역변수)에 있던 버거 인덱스를 새로운 배열에 저장 
	let products = [] ; // 새로운 주문이 들어가는 버거들 인덱스 
	for(let  i = 0 ; i<cartKust.length ; i++){
	...
	let order = {
		...
		pay:1000 , 				// 카트내 제품들의 총가격
		products: products , // 카트에 있던 모든 제품들 // 전역변수[cartList] 대입 시 문제발생
		state:0 			// 주문객체 생성시 '주문요쳥' 으로 상태 초기로 사용
	}
	// 2. 주문배열에 저장하기
	...
	// * 주문 릿스트 확인
	console.log(order); // 전체취소 함수로 동일하기 때문에 재호출
