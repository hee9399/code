 ...
 */
 ...
 //  문제 1 : 2개의 정수 를 입력받아서 가장 큰수를 출력 
 	// 1. 입력받아 변수에 저장
 		// 첫번째 값을 입력받기 (prompt) - (prompt('정수1') )
 		// 입력받은 값을 변수에 저장 a1   -  let a1 =
 		// 두번째 값을 입력받기		-	(prompt('정수2') )
 		// 입력받은 값을 변수에 저장 b1		-	let b1 =
 		// 가장큰수?? 비교/논리/조건/판단 = 연산자
 		// >= <= < > 연산자 선택
 		// if 이용한 출력
 		//
let a1 = Number(prompt('정수1 : ') )
let b1 = Number(prompt('정수2 : ') )
	// 2. if 이용한 논리제어
	// 첫번째 입력받은 데이터가 두번째 입력받은 데이터 보다 크면
if( a1 > b1 ){ console.log(a1) } // 논리(누가더 큰지에 대한 판단)
else if( a1 < b1 ){ console.log(b1) }
else{ console.log('같다') }
