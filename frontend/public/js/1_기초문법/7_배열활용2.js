/*
	변수
		지역변수 : 지역{ } 에서 태어났으면 지역 밖으로 못나감
			 - 지역변수의 저역은 { } 중괄호 를 얘기함
			ex) 안산에서 태어났으면 안산 밖으로 못나감
			- { } 중괄호 안에서 선언된 변수는 { } 밖에서 사용 불가능 / 안 으로는 가능
		전역변수 : 지역 { } 밖에서 태어났으면 자유롭게 이동 가능
			- { } 중괄호 밖에서 선언된 변수는 { } 중괄호 안으로 이동 가능 
			
		문자열내 변수 포함하는방법 [ js안에서 HTML 작성시 문자처리할때]
			1. 작은따옴표(연결연산자) = '<td>' +변수명/배열명+ '</td>'
			2. 백틱( `${}` )	= `<td>${변수명/배열명}</td>`			
*/ 
console.log('JS실행');

// 4. 배열 선언 (배열 선언은 함수 밖에 태그실행 한다)
let 방문록배열 = []; // 1. 여럿 함수에서 사용하기 때문에 함수 밖에서 선언함
	// 인덱스 = 배열내 저장된 순서 번호
