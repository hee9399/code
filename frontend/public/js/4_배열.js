 		...
 		배열 : 데이터 여러개를 저장할수 있는 공간/메모리/상자
 			...
 			7. 베얄네 데이터 추가
 					배열명.push( 새로운값 )		:	배열내 마지막 위치에 저장
 						- 마지막 인덱스 뒤에 데이터 추가
 					배열명.splice( 추가할위치 인덱스 , 0 , 새로운값 ) : 해당 위치인덱스에 새로운 값 저장
 						- 특정 위치/인덱스 에 데이터 추가
 						[ * 삭제된 요소 뒤로 한칸씩 자동 밀려남 ]
 			8. 배열내 데이터 삭제
 			...
 			9. 배열내 데이터를 이용한 데이터 삭제
 				- 데이터를 이용한 인덱스 찾기
 				배열명.indexOF('찾을데이터') : 찾을데이터가 존재한면 찾은인덱스번호 아니면 -1
...
document.write(배열명)
// 예제3 : 인덱스 사용
console.log('<h3> 과일개수 : '+배열명.length +'</h3>') 
...
배열명[0] = '수박'; 	  // 수정
console.log(배열명);    // 수정후  '수박'	

// 예제5 : 배열에 요소/데이터 추가
배열명.push('체리'); // push('체리') : 배열내 마지막 요소/데이터 뒤에 새로운 데이터 추가
console.log(배열명)
배열명.splice( 0 , 0 , '복숭아') // 0번째 인덱스에서 0개 자르고 '복숭아' 추가	 [원래있던 데이터들이 한칸씩 밀려난다]
console.log(배열명)
